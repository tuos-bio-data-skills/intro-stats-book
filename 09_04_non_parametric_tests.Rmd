...
The distribution may not be normal, but it is roughly symmetrical. The symmetry assumption of the Wilcoxon signed-rank test seems to be satisfied, so we can use this test to examine whether there is any difference in caterpillar feeding on leaves from damaged areas and undamaged areas.

As with the paired *t*-test, the Wilcoxon signed-rank test compares two measurements taken from the same experimental units — in this case, caterpillar feeding on damaged and undamaged leaves. The `Pair()` interface in R makes this pairing explicit, but it requires that each pair of measurements sits on the same row of the data set.

Our `leaves` data are currently in long format, with one row per observation and a column (`LeafType`) indicating whether the leaf was damaged or not. We can convert this to wide format using the `pivot_wider()` function from the **tidyr** package:
...
`pivot_wider()` tells R to use the values in the `LeafType` column (here, Damaged and Notdamaged) as new column names and fill those columns with the corresponding `Damage` measurements. Each sample (or leaf pair) now appears once, with both values side by side.

With the data in this format, we can perform the Wilcoxon signed-rank test using the new `Pair` syntax:

```{r}
wilcox.test(Pair(Notdamaged, Damaged) ~ 1, data = leaves_wide)
```
...
The *p*-value is close to *p* = 0.05, but it doesn't quite fall below the threshold, i.e. there is no significant difference in feeding rates on the two types of leaf. We might report this as:
...
Let's use the Red wood ants example from the chapter on transformations to see how to carry out the Mann-Whitney *U*-test. Recall that this study measured the total biomass of prey being transported---the rate of food collection per ant per half hour---on three different tree species (rowan, sycamore and oak). Since the Mann-Whitney *U*-test only compares two samples, we will only use on the oak and sycamore cases to illustrate the test[^09_04_non_parametric_tests-2].

[^09_04_non_parametric_tests-2]: Of course, if we really were interested in the difference between sycamore, oak *and* rowan we should use a different test!
