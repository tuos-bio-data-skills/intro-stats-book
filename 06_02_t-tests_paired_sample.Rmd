...
### Using the new `Pair()` interface for paired *t*-tests
...
Our original `glycolipid` data are in *long* format: each row represents a single measurement for a patient, with one column for the drug name and another for the glycolipid level. To use the new `Pair()` interface, we first need to reshape the data into *wide* format, where each patient’s two measurements occupy a single row — one column for Drug A and one for Drug B.

We can do this with the `pivot_wider()` function from the tidyverse **tidyr** package:
